...
describe("Accepts options", () => {
  ...
  describe("Accepts custom cacheMap instance", () => {
    class SimpleMap<K, V> {
      stash: Record<string, V>;

      constructor() {
        this.stash = {};
      }
      get(key: K) {
        return this.stash[String(key)];
      }
      set(key: K, value: V) {
    ...
    it("Accepts a custom cache map implementation", async () => {
      const aCustomMap = new SimpleMap<string, Promise<string>>();
      const identityLoadCalls: string[][] = [];
...
describe("It is resilient to job queue ordering", () => {
  ...
  it("can call a loader from a loader", async () => {
    const deepLoadCalls: (readonly string[][])[] = [];
    const deepLoader = new DataLoader<readonly string[], readonly string[]>(
      (keys) => {
        deepLoadCalls.push([...keys] as string[][]);
        return Promise.resolve([...keys]);
      },
    );
